...
const UPSTREAM_REDEEM =
  process.env.REDEEM_API ||
  'https://ttt9-9-9.onrender.com/api/redeem';
...
async function initDb() {
  ...
  await q(`
    CREATE INDEX IF NOT EXISTS tx_bets_user_idx
    ON tx_bets(user_id, id DESC)
  `);

  await q(`
    CREATE INDEX IF NOT EXISTS tx_history_id_idx
    ON tx_history(id DESC)
  `);
...
async function settleRound(round) {
  ...
  try {
    ...
    const resultUntil = new Date(
      Date.now() + RESULT_GAP_SECONDS * 1000
    );
    ...
    for (const bet of bets.rows) {
      const win =
        bet.side === (side === 'TÀI' ? 'tai' : 'xiu')
          ? Math.floor(Number(bet.amount) * PAYOUT)
          : 0;
...
async function gameTick() {
  try {
    ...
    if (
      ...
      r.status === 'closed' &&
      r.result_until &&
      new Date(r.result_until).getTime() <= t
    ) {
      await createRound();
    }
  ...
  try {
    ...
    res.json({
      ...
      user: {
        ...
        ttt: r.rows[0].ttt,
        spins: r.rows[0].spins,
        best_win: r.rows[0].best_win
      }
    });
...
app.get('/api/tx/state', auth, async (req, res) => {
  ...
  res.json({
    ...
    round: {
      ...
      resultUntil: r.result_until
        ? new Date(r.result_until).getTime()
        : null,
      status: r.status,
      dice: r.status === 'closed'
        ? [r.d1, r.d2, r.d3]
        : null,
      total: r.total || null,
      side: r.side || null
    ...
    myStake: Number(my.rows[0].amount),
...
async function xdTick() {
  ...
  try {
    ...
    if (
      ...
      await settleXdRound(r);
    }

  } catch (e) {
    console.error('xdTick:', e);
  }
}
...
app.post('/api/redeem', auth, async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({
      ok: false,
      error:
        'Không kết nối được máy chủ code.'
    });
  }
});
...
async function start() {
  ...
  try {
    ...
    app.listen(PORT, () => {
      console.log(
        `SCT Rank Arena server listening on ${PORT}`
      );
    });
